...
# This function sets the excel_file argument to 'url'
# which points to the Excel file. I will then
# map over 'sheet'
...
# Inspired by the `clean_names()` function from the `{janitor}` R package.
...
# - converting columns to their right types
# - filtering out rows with irrelevant text
raw_data = (
    raw_data
    .rename(
      {
    ...
    .filter(
      pl.col("n_offers") != "Nombre d'offres"
    )
    .with_columns(
      pl.col("n_offers").cast(pl.Int64)
    )
)
...
believe that reading the code above should be easy. But in any case,
rembemer that this book is not about Python per se, but about building
reproducible pipelines. The focus will be on other things.
