...
```bash
micromamba create -n housing python=3.11.9 pipenv
```

This will create an environment named "pipenv_env" that includes `pipenv` and
the version 3.11.9 of Python. If you're just starting a project, you can safely
...
you'll notice that here the Python version is "3.11", but we specified version
"3.11.9" with `micromamba` when we created the environment. I would recommend
that you add the missing ".9" for maximum reproducibility. If you edited the `Pipfile`
...
This should print `Python 3.11.9` in the terminal. Start the Python interpreter
...
You should see `1.1.0` as the listed version. Quite the shell, and then
quite the environment with `exit`.
...
we navigate to the `housing` folder, the `housing` environment starts. We
can achieve this by using a tool called `direnv`.
